// afk-core/src/profile/elementalmc/lifecycle.js
"use strict";

// ElementalMC lifecycle logger (ALWAYS ON, not gated by ELEMENTALMC_DEBUG).
...
function installLifecycleLogger(bot, entry) {
  const _life = (msg) =>
    console.log(`[elem-life] ${new Date().toISOString()} [${entry.minecraftUser}] ${msg}`);

  // Incoming packet logger (forensic mode only).
  if (ELEMENTALMC_DEBUG) {
    bot._client.on("packet", (data, meta) => {
      try {
        console.log(`[elem-pkt] ← SERVER [${meta.state}]: ${meta.name}`, JSON.stringify(data).slice(0, 160));
      } catch {
        console.log(`[elem-pkt] ← SERVER [${meta.state}]: ${meta.name} (binary)`);
      }
    });
  }
  ...
  bot._client.on("state", (s) => {
    ...
    if (s === "configuration" && _recentOut.length) {
      ...
      for (const e of _recentOut) {
        console.log(`[elem-life]    ${e.t}  → ${e.name} ${e.brief}`);
      }
      _recentOut.length = 0;
    }
  });
