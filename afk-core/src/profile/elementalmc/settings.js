// afk-core/src/profile/elementalmc/settings.js
"use strict";

// ElementalMC outgoing-write interceptor + client-settings builder.
//
...
const ELEMENTALMC_DEBUG =
  String(process.env.ELEMENTALMC_DEBUG || "minimal").toLowerCase() === "forensic";
...
function installSettingsInterceptor(bot, entry) {
  ...
  bot._client.write = function elementalMcSettingsPatch(name, params) {
    ...
    if (
      ...
      if (ELEMENTALMC_DEBUG) {
        console.log(`[elem-pkt] → DROPPED ${name} (movement during configuration state)`);
      }
      return;
    }
    ...
    if (name === "settings" || name === "client_information") {
      ...
      console.log(`[ElementalMcProfile] ⚙️ [${entry.minecraftUser}] ${name} intercepted → fields ensured`);
    }

    if (ELEMENTALMC_DEBUG) {
      const st = bot._client.state;
      try {
        console.log(`[elem-pkt] → CLIENT sent [${st}]: ${name}`, JSON.stringify(params).slice(0, 160));
      } catch {
        console.log(`[elem-pkt] → CLIENT sent [${st}]: ${name} (non-serializable)`);
      }
    }
    ...
    // Outgoing packet recorder (ring buffer dumped by lifecycle.js on kick).
    {
      ...
      if (!ROUTINE_OUTGOING.has(name)) {
        console.log(`[elem-life] ${new Date().toISOString()} [${entry.minecraftUser}] → SENT ${name} ${brief}`);
      }
    }
...
module.exports = { sendClientSettings, installSettingsInterceptor, ELEMENTALMC_DEBUG, ROUTINE_OUTGOING };
