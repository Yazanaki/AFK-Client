// afk-core/src/profile/elementalmc/keepAlive.js
"use strict";

// ElementalMC connection handshake / keepalive. We run keepAlive:false, so we echo
...
function installKeepAlive(bot, entry, origWrite) {
  ...
  bot._client.on("ping", (packet) => {
    if (++_pingSeen <= 3) {
      console.log(
        `[elem-life] ${new Date().toISOString()} [${entry.minecraftUser}] ` +
  ...
  bot._client.on("cookie_request", (packet) => {
    try {
      ...
      console.log(`[ElementalMcProfile] 🍪 [${entry.minecraftUser}] cookie_response sent for key=${packet.key}`);
    } catch (_) {}
  });
}
