// afk-core/src/profile/elementalmc/reconnect.js
"use strict";

// ElementalMC transient-kick reconnect.
//
// ElementalMC's transaction / anti-cheat check can kick with "Invalid sequence"
...
// NOT port DonutSMP's movement-freeze — ElementalMC *requires* per-tick movement
...
function scheduleReconnect(entry, spawnBot, version) {
  ...
  if (entry.profileReconnectRetries > MAX_RECONNECT_RETRIES) {
    console.error(
      `[ElementalMcProfile] ❌ [${entry.minecraftUser}] Reconnect retry limit reached — giving up`
    );
    entry.status = "error";
    entry.errorCategory = "elementalmc_invalid_sequence";
    entry.spawnError =
      "ElementalMC kept rejecting the connection ('invalid sequence') after multiple " +
      "retries. Wait a moment and try /mcbot start again.";
    return;
  }

  console.log(
    `[ElementalMcProfile] 🟠 [${entry.minecraftUser}] Transient kick — reconnect ` +
