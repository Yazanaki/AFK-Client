// afk-core/src/profile/elementalmc/hunger.js
"use strict";

// ElementalMC auto-eat. Self-contained copy (see also donutsmp/hunger.js).
//
...
function attachHunger(bot, entry) {
  ...
  async function tryEat() {
    ...
    try {
      ...
      console.log(`[elementalmc/hunger] 🍖 ${entry.minecraftUser} ate ${foodItem.name} (food: ${bot.food}/20)`);
    } catch (err) {
      console.warn(`[elementalmc/hunger] ⚠️ Eat failed for ${entry.minecraftUser}:`, err.message);
